 ...
 */
MAX30101::MAX30101(uint8_t addr, TwoWire &wire)
...
bool MAX30101::setDefaultConfiguration() {
  ...
  if (!setMultiLedConfiguration(cfg))
    return false;
  if (!setLedCurrent(LED_RED, 90))
    return false;
  if (!setLedCurrent(LED_IR, 80))
    return false;
  if (!setLedCurrent(MAX3010X_LED_GREEN_CH1, 100))
    return false;
  if (!setLedCurrent(MAX3010X_LED_GREEN_CH2, 0))
    return false;
  if (!setResolution(RESOLUTION_18BIT_4110US))
    return false;
  if (!setSamplingRate(SAMPLING_RATE_50SPS))
    return false;
  if (!setSampleAveraging(SMP_AVE_NONE))
    return false;
  if (!setADCRange(ADC_RANGE_16384NA))
    return false;
  if (!enableFIFORollover())
    return false;
  if (!setMode(MODE_SPO2))
    return false;
 ...
 * @returns true if successful, otherwise false
 */
bool MAX30101::setMultiLedConfiguration(
    const MAX30101::MultiLedConfiguration &cfg) {
  // Count active slots and ensure that slots are enable in order
  uint8_t activeSlots = 0;
  for (int i = 0; i < 4; i++) {
    if (static_cast<uint8_t>(cfg.slot[i]) > 0b100)
      return false;

    if (cfg.slot[i] == SLOT_RED || cfg.slot[i] == SLOT_IR ||
        cfg.slot[i] == SLOT_GREEN) {
      if (activeSlots != i)
        return false;
      activeSlots++;
    }
  ...
  uint8_t rawCfg[2]{};
  rawCfg[0] = static_cast<uint8_t>(cfg.slot[0]) |
              (static_cast<uint8_t>(cfg.slot[1]) << 4);
  rawCfg[1] = static_cast<uint8_t>(cfg.slot[2]) |
              (static_cast<uint8_t>(cfg.slot[3]) << 4);
