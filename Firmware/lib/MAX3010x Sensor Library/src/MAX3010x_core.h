...
protected:
  ...
  static const uint8_t FIFO_WR_PTR_REG =
      MAX3010xImpl::FIFO_BASE; //!< FIFO Write Pointer Register
  static const uint8_t FIFO_OVF_CNT_REG =
      MAX3010xImpl::FIFO_BASE + 1; //!< FIFO Overflow Counter Register
  static const uint8_t FIFO_RD_PTR_REG =
      MAX3010xImpl::FIFO_BASE + 2; //!< FIFO Read Pointer Register
  static const uint8_t FIFO_DATA_REG =
      MAX3010xImpl::FIFO_BASE + 3; //!< FIFO Data Register
  ...
  bool readBlock(uint8_t reg, uint8_t count, uint8_t *buffer) {
    ...
    if (_wire.requestFrom(_addr, count) != count)
      return false;
   ...
   * @return true if successful, otherwise false
   */
  bool waitBit(uint8_t reg, uint8_t bit, bool expectedState = true,
               int timeout = 100) {
  ...
  bool setModeInternal(uint8_t mode) {
    ...
    if (mode & (~MODE_MASK))
      return false;
    if (!readByte(MAX3010xImpl::MODE_CFG_REG, value))
      return false;

    value &= ~MODE_MASK;
    value |= mode;

    if (!writeByte(MAX3010xImpl::MODE_CFG_REG, value))
      return false;
    return clearFIFO();
  }
...
public:
  ...
  bool reset() {
    ...
    // Reset
    if (!setBit(MAX3010xImpl::MODE_CFG_REG, MAX3010xImpl::MODE_RST_BIT, true))
      return false;
    if (!waitBit(MAX3010xImpl::MODE_CFG_REG, MAX3010xImpl::MODE_RST_BIT, false))
      return false;

    // Identify part
    if (readPartId() != MAX3010xImpl::MAX3010x_PART_ID)
      return false;

    // Enable Temperature Interrupt
    if (!enableInterrupt(MAX3010xImpl::INT_TEMP_RDY))
      return false;

    // Default Config
    if (!static_cast<MAX3010xImpl *>(this)->setDefaultConfiguration())
      return false;
  ...
  bool enableInterrupt(uint8_t interrupt) {
    if (interrupt >= MAX3010xImpl::INT_CNT)
      return false;
    if (MAX3010xImpl::INT_CFG_REG[interrupt] == 0xFF ||
        MAX3010xImpl::INT_CFG_BIT[interrupt] >= 8)
      return false;
    return setBit(MAX3010xImpl::INT_CFG_REG[interrupt],
                  MAX3010xImpl::INT_CFG_BIT[interrupt], true);
  }
   ...
   * @param interrupt Interrupt
   * @return true if successful, otherwise false
   * @remarks If you disable the temperature interrupt the readTemperature()
  ...
  bool disableInterrupt(uint8_t interrupt) {
    ...
    if (MAX3010xImpl::INT_CFG_REG[interrupt] == 0xFF ||
      ...
      return false;
    return setBit(MAX3010xImpl::INT_CFG_REG[interrupt],
                  MAX3010xImpl::INT_CFG_BIT[interrupt], false);
  }
  ...
  bool checkInterruptFlag(uint8_t interrupt) {
    ...
    if (interrupt >= MAX3010xImpl::INT_CNT)
      return false;
    if (!readBit(MAX3010xImpl::INT_ST_REG[interrupt],
                 MAX3010xImpl::INT_ST_BIT[interrupt], value))
      return false;
    return value;
  }
  ...
  bool waitForInterrupt(uint8_t interrupt, int timeout = 100) {
    if (interrupt >= MAX3010xImpl::INT_CNT)
      return false;
    return waitBit(MAX3010xImpl::INT_ST_REG[interrupt],
                   MAX3010xImpl::INT_ST_BIT[interrupt], true, 100);
  }
  ...
  uint8_t readPartId() {
    uint8_t partId;
    if (!readByte(PART_ID_REG, partId))
      return 0xFF;
    return partId;
  }
  ...
  uint8_t readRevisionId() {
    uint8_t revisionId;
    if (!readByte(REV_ID_REG, revisionId))
      return 0xFF;
    return revisionId;
  }
   ...
   */
  bool shutdown() {
    return setBit(MAX3010xImpl::MODE_CFG_REG, MAX3010xImpl::MODE_SHDN_BIT,
                  true);
  }
   ...
   * @remarks
   * This function will trigger the temperature interrupt once the conversion is
   * finished. Triggering this interrupt is necessary for this method. If you
   * use the interrupt pin of the MAX3010x in your application please be aware
   * of this fact.
  ...
  float readTemperature() {
    ...
    if (!setBit(MAX3010xImpl::TEMP_CONFIG_REG, MAX3010xImpl::TEMP_CONFIG_BIT,
                true))
      return NAN;
    if (!waitForInterrupt(MAX3010xImpl::INT_TEMP_RDY))
      return NAN;
    if (!readByte(MAX3010xImpl::TINT_REG, tInt) ||
        !readByte(MAX3010xImpl::TFRAC_REG, tFrac))
      return NAN;
  ...
  uint8_t available() {
    FIFORegisters fifo;
    if (!readBlock(MAX3010xImpl::FIFO_BASE, sizeof(FIFORegisters),
                   reinterpret_cast<uint8_t *>(&fifo)))
      return 0;
  ...
  uint8_t readOverflowCounter() {
    ...
    if (!readByte(FIFO_OVF_CNT_REG, overflowCounter))
      return 0xFF;
   ...
   */
  bool clearFIFO() {
    if (!writeByte(MAX3010xImpl::FIFO_WR_PTR_REG, 0))
      return false;
    if (!writeByte(MAX3010xImpl::FIFO_RD_PTR_REG, 0))
      return false;
    if (!writeByte(MAX3010xImpl::FIFO_OVF_CNT_REG, 0))
      return false;
    return true;
  }
  ...
  MAX3010xSample readSample(int timeout = 0) {
    ...
    uint8_t data[MAX3010xImpl::SAMPLE_SIZE * MAX3010xImpl::MAX_ACTIVE_LEDS] = {
        0};
    if (!readBlock(MAX3010xImpl::FIFO_DATA_REG,
                   MAX3010xImpl::SAMPLE_SIZE *
                       static_cast<MAX3010xImpl *>(this)->nActiveSlots,
                   data)) {
